...
export function LightWavesBackground({
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            if (isDark) {
                ...
                bgGradient.addColorStop(0.5, "#e2e8f0") // Slate 200
            ...
            for (const spot of glowSpots) {
                ...
                if (isDark) {
                    ...
                    // Lighter/More subtle glow for light mode
                    gradient.addColorStop(0, `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${0.05 * intensity})`)
                    gradient.addColorStop(0.5, `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${0.01 * intensity})`)
                }
            ...
            for (const wave of wavesRef.current) {
                ...
                // Use slightly different opacities for light mode blending
                const waveOpacity = isDark ? wave.opacity : wave.opacity * 0.8

                waveGradient.addColorStop(
                    0,
                    `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${waveOpacity * intensity})`,
                )
                waveGradient.addColorStop(
                    0.3,
                    `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${waveOpacity * 0.5 * intensity})`,
                )
    ...
    return (
        <div ref={containerRef} className={cn("relative h-full w-full overflow-hidden", className)}>
            ...
            {/* Vignette - Adjusted for theme handled by CSS if needed, but here simple opacity trick */}
            <div
                className="pointer-events-none absolute inset-0 transition-opacity duration-300"
                style={{
                    background: "radial-gradient(ellipse at center, transparent 0%, transparent 50%, var(--vignette-color, rgba(3,7,18,0.7)) 100%)"
                }}
            >
                <style>{`
                    .dark .relative { --vignette-color: rgba(3,7,18,0.7); }
                    :root { --vignette-color: rgba(0,0,0,0.1); }
                `}</style>
            </div>
