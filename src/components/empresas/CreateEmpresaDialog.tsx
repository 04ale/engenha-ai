...
export function CreateEmpresaDialog({
    ...
    const fetchCompanyData = async () => {
        ...
        try {
            ...
            toast.error(
                "Não foi possível carregar os dados da empresa. Tente preencher manualmente."
            )
        } finally {
    ...
    const onSubmit = async (data: EmpresaInput) => {
        if (!user?.workspace_id || !user?.id) {
            toast.error("Você precisa estar autenticado a um workspace")
            return
        }
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent className="sm:max-w-[600px] mx-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Cadastre uma nova empresa para vincular à obra.
                    </DialogDescription>
                </DialogHeader>
