...
export function exportItemTemplate(): void {
  const template: ExcelItemRow[] = [
    ...
    },
    {
      ...
      unidade: "m³",
      quantidade: 50,
      valor_executado: 350.00,
      data_execucao: "2024-01-20",
    },
  ]

  const worksheet = XLSX.utils.json_to_sheet(template, {
    header: ["descricao", "unidade", "quantidade", "valor_executado", "data_execucao"],
  })
        ...
        const items: CreateItemInput[] = normalizedData
          .filter((row) => {
            ...
            return (
              ...
              row.descricao.trim() !== "" &&
              !row.descricao.toLowerCase().includes("exemplo")
            )
          })
          .map((row) => {
            ...
            if (typeof data_execucao === "number") {
              ...
              const dateMatch = data_execucao.match(/(\d{2})\/(\d{2})\/(\d{4})/)
              if (dateMatch) {
            ...
            if (!row.descricao || !row.unidade) {
              throw new Error(`Linha inválida: descrição e unidade são obrigatórias`)
            }
...
export function exportItemsToExcel(items: CreateItemInput[]): void {
  ...
  XLSX.writeFile(workbook, `itens_acervo_${new Date().toISOString().split("T")[0]}.xlsx`)
}
